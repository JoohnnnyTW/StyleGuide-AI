...
import { CameraIcon as DefaultCameraIcon } from 'lucide-react'; // Import a default if needed
...
// Props for the main AnimatedTabs coordinator component
...
// Props for the new AnimatedTabGroup sub-component
interface AnimatedTabGroupProps {
  tabs: TabDef[];
  activeTab: string;
  onTabChange: (label: string) => void;
  isFirstGroup: boolean; // To adjust rounding for the very first group
  isLastGroup: boolean;  // To adjust rounding for the very last group
}

const TAB_NAME_CAMERA = "相機"; // As defined in App.tsx

// Sub-component to render a group of non-camera tabs with a sliding background
const AnimatedTabGroup: React.FC<AnimatedTabGroupProps> = ({ tabs, activeTab, onTabChange, isFirstGroup, isLastGroup }) => {
    ...
    if (isActiveTabInThisGroup) {
      ...
      if (activeEl && container.offsetWidth > 0) {
        ...
        if (containerRect.width > 0) {
          ...
          // Clamp values and check for invalid states (e.g., button outside container, or sum > 100%)
          ...
          if (clipLeftPercent + clipRightPercent >= 100) {
            // This case means the button is effectively not visible or wider than container, or calculation error
            // Default to hiding the highlight for this state
          ...
          // Container has no width, hide highlight
          container.style.clipPath = defaultHideClipPath;
        }
      } else {
        // Active tab not found or container has no width, hide highlight
        container.style.clipPath = defaultHideClipPath;
      }
    } else {
      // Active tab is not in this group, hide highlight
      container.style.clipPath = defaultHideClipPath;
    }
  }, [activeTab, tabs]);
...
export function AnimatedTabs({ tabs, activeTab, onTabChange }: AnimatedTabsProps) {
  ...
  if (cameraTabIndex !== -1) {
    ...
    // No camera tab, all tabs go into one group (like previous behavior)
  ...
  const CameraIconToUse = cameraTabDetails?.icon || DefaultCameraIcon; // Fallback to a default camera icon if not provided
      ...
      <div className="flex items-end gap-2 sm:gap-3 pointer-events-auto">
        ...
        {tabsBeforeCamera.length > 0 && (
          <AnimatedTabGroup 
            ...
            isFirstGroup={true}
            isLastGroup={!cameraTabDetails && tabsAfterCamera.length === 0} 
          />
        )}
        ...
        {cameraTabDetails && (
          <button
            ...
            className={`
              h-[36px] w-[36px] sm:h-[42px] sm:w-[42px] /* Adjusted size for ~1.5x */
          ...
          >
            <CameraIconToUse className="w-4 h-4 sm:w-5 sm:w-5" /> {/* Icon size can be adjusted if needed */}
          </button>
        )}
        ...
        {tabsAfterCamera.length > 0 && (
          <AnimatedTabGroup 
            ...
            activeTab={activeTab} 
            onTabChange={onTabChange}
            isFirstGroup={!cameraTabDetails && tabsBeforeCamera.length === 0}
            isLastGroup={true}
          />
        )}
      </div>
