...
const TrashIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className || "w-3.5 h-3.5 sm:w-4 sm:h-4"}>
...
export const AppSettingsModal: React.FC<AppSettingsModalProps> = ({
    ...
    const handleSafetyToleranceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isNaN(value)) value = 2; // Default if parsing fails
        if (value < 0) value = 0;
        if (value > 5) value = 5;
      ...
      >
        ...
        >
          <header className="p-4 sm:p-5 border-b border-gray-200 dark:border-neutral-700 flex justify-between items-center">
            <h2 id="app-settings-modal-title" className="text-lg sm:text-xl font-semibold text-neutral-800 dark:text-neutral-100">
              應用程式設定
            </h2>
            <button
              onClick={onClose}
                    ...
                    <>
                        ...
                        <div className="p-2 rounded-md bg-neutral-50 dark:bg-neutral-700/50 border border-neutral-200 dark:border-neutral-600">
                            <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-300">
                                Imagen 3 使用應用程式預設的 Gemini API 金鑰。請確保已在伺服器端環境中設定金鑰。
                            </p>
                            {!isGeminiApiAvailable && (
