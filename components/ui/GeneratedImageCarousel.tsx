...
import { GeneratedImageHistoryEntry } from '../../App'; // Path might need adjustment if App.tsx moved
import { FileTextIcon } from 'lucide-react';

interface GalleryItemProps {
  item: GeneratedImageHistoryEntry;
  onImageSelect: (item: GeneratedImageHistoryEntry) => void; // For lightbox
...
const GalleryItem: React.FC<GalleryItemProps> = ({ item, onImageSelect, onUseAsPrompt, onAddToMain, onGenerateReport, isGeneratingReport }) => {
  ...
  return (
    ...
    >
      ...
      />
      {/* Overlay for prompt text and action buttons */}
      <div className="absolute inset-0 flex flex-col justify-between p-2 sm:p-3 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <div className="mt-auto pt-1.5 sm:pt-2 flex flex-wrap gap-1 sm:gap-1.5 justify-end">
            <button
                ...
                className="px-2 py-0.5 text-[0.6rem] sm:px-2.5 sm:py-1 sm:text-[10px] md:text-xs bg-sky-500/90 text-white rounded hover:bg-sky-600 shadow-md transition-all hover:scale-105 active:scale-95 focus:outline-none focus:ring-1 focus:ring-sky-300 disabled:opacity-60 disabled:cursor-wait"
            ...
            <button
                ...
                className="px-2 py-0.5 text-[0.6rem] sm:px-2.5 sm:py-1 sm:text-[10px] md:text-xs bg-green-500/90 text-white rounded hover:bg-green-600 shadow-md transition-all hover:scale-105 active:scale-95 focus:outline-none focus:ring-1 focus:ring-green-300 disabled:opacity-60 disabled:cursor-wait"
            ...
            {onGenerateReport && (
                 <button
                    ...
                    disabled={isGeneratingReport && item.id === (window as any).generatingReportId} // More specific disabling
                >
                    {(isGeneratingReport && item.id === (window as any).generatingReportId) ?
                         <svg className="animate-spin h-2.5 w-2.5 sm:h-3 sm:w-3 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        : <FileTextIcon className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                    }
                    報告
                </button>
            )}
       ...
       {/* Fallback for non-hover state to show a snippet of the prompt */}
...
export const GeneratedImageGallery: React.FC<GeneratedImageGalleryProps> = ({ history, onImageSelect, onUseAsPrompt, onAddToMain, onGenerateReport, isGeneratingReport }) => {
