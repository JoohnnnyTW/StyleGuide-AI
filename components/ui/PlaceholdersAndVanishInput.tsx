...
import { PaperclipIcon } from './PaperclipIcon'; // Assuming PaperclipIcon.tsx is in the same directory
...
export const PlaceholdersAndVanishInput: React.FC<PlaceholdersAndVanishInputProps> = ({
  ...
  const isButtonEffectivelyDisabled = isSubmitDisabled; // App.tsx now handles disabling logic more comprehensively
  ...
  const handlePromptImageFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (promptImageInputRef.current) {
      promptImageInputRef.current.value = ""; // Reset file input
    }
  };
  ...
  return (
    <>
      <form
        ...
        className="relative w-full rounded-full p-0.5 sm:p-1 bg-white shadow-sm border border-[#EAEAEA] focus-within:border-neutral-700 transition-colors duration-300"
      >
        <div className="relative flex items-center w-full">
          <button
            type="button"
            onClick={() => promptImageInputRef.current?.click()}
            className="p-2 sm:p-2.5 ml-1 sm:ml-1.5 rounded-full text-neutral-500 hover:text-neutral-700 hover:bg-neutral-100 focus:outline-none focus:ring-1 focus:ring-sky-500"
            aria-label="附加圖片至提示"
          >
          ...
          <input
            ...
            className={`w-full h-10 sm:h-12 pr-10 sm:pr-12 text-sm sm:text-base text-[#1D1D1F]/90 bg-transparent rounded-full outline-none focus:ring-0 border-none placeholder:text-[#1D1D1F]/60 
                        ${promptImagePreviewUrl ? 'pl-1.5 sm:pl-2' : 'pl-2 sm:pl-3'}`} // Adjust padding based on preview
          ...
          {showAnimatedPlaceholder && (
            <span
              ...
              className={`absolute top-1/2 -translate-y-1/2 pointer-events-none 
                          text-sm sm:text-base text-[#1D1D1F]/60 
                          transition-opacity duration-500 ease-in-out 
                          ${placeholderOpacityClass}
                          ${promptImagePreviewUrl ? 'left-[4.5rem] sm:left-24' : 'left-12 sm:left-14'}`} // Adjust placeholder position
            >
          ...
          <button
            ...
            className={`absolute right-0.5 sm:right-1 top-1/2 -translate-y-1/2 p-2 sm:p-2.5 rounded-full text-white 
                       bg-[#1D1D1F] hover:bg-neutral-700 
                       focus:outline-none focus:ring-2 focus:ring-neutral-600 focus:ring-opacity-50 
