...
import { RefreshCwIcon, TagsIcon, BrainIcon, InfoIcon, CheckCircleIcon, FileTextIcon, CameraIcon, XIcon, DownloadIcon as LucideDownloadIcon, SendIcon } from 'lucide-react';
...
// Ensure ImageDetail type used in App.tsx includes displayName
type ImageDetail = ModalImageDetail;
type SubmissionEntry = ModalSubmissionEntry;

// For "Upload Main Image(s)" - these are the reference/auxiliary images
interface SelectedImageDataPerTab {
  id: string; // Unique ID for keying in React loops
  file: File;
  previewUrl: string;
  tags: string[]; // General tags added via the dropdown
  displayName:string;
  currentDropdownTagSelection?: string; // For the general tag adding dropdown
  aiGeneratedTags?: string[]; // Tags generated by AI OCR
  isFetchingOcr?: boolean;    // Is OCR in progress for this image?
  ocrError?: string | null;   // OCR error message for this image
  manualTagInput?: string; // For new "參考" (old "生圖") mode manual tag entry
}
...
const TAB_NAME_GENERATE_IMAGE = "參考"; // Formerly "生圖"
...
const CloseIcon: React.FC<{ className?: string; strokeWidth?: number }> = ({ className, strokeWidth = 2.5 }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={strokeWidth} stroke="currentColor" className={className || "w-4 h-4 sm:w-5 sm:h-5"}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
  </svg>
);

const UploadIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className={className || "w-4 h-4 sm:w-5 sm:h-5"}>
...
const ChevronUpIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={className || "w-4 h-4 sm:w-5 sm:h-5"}>
...
export const AppleIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg fill="currentColor" viewBox="0 0 24 24" {...props} className={props.className || "w-5 h-5 sm:w-6 sm:h-6"}>
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                if (currentRatio > targetAspectRatioVal) { // Original is wider than target relative to height, crop width
                    sWidth = originalHeight * targetAspectRatioVal;
                    sx = (originalWidth - sWidth) / 2;
                } else if (currentRatio < targetAspectRatioVal) { // Original is taller than target relative to width, crop height
...
const App: React.FC = () => {
  ...
  const [submittedEntries, setSubmittedEntries] = useState<SubmissionEntry[]>([]);
  ...
  const [imagenOutputFormat, setImagenOutputFormat] = useState<string>(() => localStorage.getItem('imagenOutputFormat') || 'image/jpeg'); // 'image/jpeg' or 'image/png'
  const [fluxOutputFormat, setFluxOutputFormat] = useState<string>(() => localStorage.getItem('fluxOutputFormat') || 'png'); // 'png' or 'jpeg'
  const [fluxPromptUpsampling, setFluxPromptUpsampling] = useState<boolean>(() => (localStorage.getItem('fluxPromptUpsampling') === 'true') || false); // Default false
  const [fluxSafetyTolerance, setFluxSafetyTolerance] = useState<number>(() => parseInt(localStorage.getItem('fluxSafetyTolerance') || '2', 10)); // Default 2
    ...
    try {
      ...
      try {
        ...
        if (responseText && responseText.length < 200) { 
            errorMsg += ` 回應：「${responseText.substring(0,100)}${responseText.length > 100 ? "..." : ""}」`;
...
Respond ONLY with the JSON array of the new tags.`;
  ...
  const logCurrentStateToHistory = () => {
    const imagesForHistory = activeAnimatedTab !== TAB_NAME_REFERENCE_INSPIRATION && activeAnimatedTab !== TAB_NAME_CAMERA
      ? (tabSelectedImageData[activeAnimatedTab] || [])
      : [];
    
    if (!currentInputTextForDisplay.trim() && imagesForHistory.length === 0) {
        alert("請提供文字提示或上傳主要圖片以新增至歷史紀錄。");
        return;
    }
    let newImagesData: ImageDetail[] | null = null;
    if (imagesForHistory.length > 0) {
        newImagesData = imagesForHistory.map(data => ({
            url: data.previewUrl,
            tags: [...data.tags, ...(data.aiGeneratedTags || [])], 
            displayName: data.displayName,
        }));
    }
    setSubmittedEntries(prevEntries => [...prevEntries, { text: currentInputTextForDisplay, images: newImagesData }]);
    setUserBasePrompt("");
    setSelectedAiSuggestionValues([]);
    if (activeAnimatedTab === TAB_NAME_GENERATE_IMAGE) setGenModeSelectedTagsForPrompt([]);
    clearImagesInApp(activeAnimatedTab); 
    clearPromptSpecificImage(); 
    setAiSuggestions([]); 
  };
  ...
  const openSubmittedHistoryModal = () => setIsModalOpen(true);
  const closeSubmittedHistoryModal = () => setIsModalOpen(false);
  ...
  const clearImagesInApp = (tabKey: string) => {
    if (tabKey === TAB_NAME_REFERENCE_INSPIRATION || tabKey === TAB_NAME_CAMERA) return; 

    (tabSelectedImageData[tabKey] || []).forEach(data => {
      if (data.previewUrl.startsWith('blob:')) URL.revokeObjectURL(data.previewUrl);
    });
    setTabSelectedImageData(prev => ({ ...prev, [tabKey]: [] }));
    if (fileInputRef.current) fileInputRef.current.value = "";
    if (tabKey === TAB_NAME_GENERATE_IMAGE) setGenModeSelectedTagsForPrompt([]); 
  };
...
${reportData.suggestions.map(s => `- ${s}`).join('\n')}
  ...
  useEffect(() => {
    return () => {
      submittedEntries.forEach(entry => {
        if (entry.images) entry.images.forEach(imgDetail => {
          if (imgDetail.url && imgDetail.url.startsWith('blob:')) URL.revokeObjectURL(imgDetail.url);
        });
      });
      Object.values(tabSelectedImageData).flat().forEach(data => {
        if (data.previewUrl && data.previewUrl.startsWith('blob:')) URL.revokeObjectURL(data.previewUrl);
      });
      if (promptSpecificImagePreviewUrl && promptSpecificImagePreviewUrl.startsWith('blob:')) {
        URL.revokeObjectURL(promptSpecificImagePreviewUrl);
      }
      stopCamera(); 
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [submittedEntries, tabSelectedImageData, promptSpecificImagePreviewUrl]); 
  ...
  if (isLoading) {
    return <SplashScreen appName="StyleGuide AI" tagline="Crafting Visual Excellence..." />;
  }
    ...
    return (
      <div className="w-full flex flex-col items-center pb-20 sm:pb-24"> 
        ...
        <div className="w-full max-w-xl mt-auto"> 
              ...
              <div className="mb-4 sm:mb-6 p-3 sm:p-4 border border-gray-200 dark:border-neutral-700 rounded-lg shadow-sm bg-gray-50 dark:bg-neutral-800/30 flex flex-col items-center justify-center min-h-[150px] sm:min-h-[200px]">
                  ...
                  {generatedImageUrl && !isGeneratingImage && !generationError && (
                    <div className={`mt-2 sm:mt-3 flex flex-wrap gap-1.5 sm:gap-2 justify-center ${showImageCompare ? 'pt-2 border-t border-neutral-200 dark:border-neutral-700 w-full max-w-lg' : ''}`}>
                        ...
                        >
                            {isGeneratingReport ? 
                                <svg className="animate-spin h-3 w-3 sm:h-3.5 sm:w-3.5 text-indigo-700 dark:text-indigo-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                                : <FileTextIcon className="w-3 h-3 sm:w-3.5 sm:w-3.5" />
                            }
          ...
          {!ai && selectedEngine === 'imagen' && <p className="text-center text-xs text-red-500 dark:text-red-400 mt-1.5 sm:mt-2">Imagen 3 (Gemini) API 金鑰未設定。圖片生成將無法使用 Imagen 3。請檢查環境設定或選擇 Flux 並提供金鑰。</p>}
  ...
  const isAnyModalOpen = isAppSettingsModalOpen || isLoginModalOpen || isModalOpen || isGeneratedHistoryModalOpen || isReportModalOpen;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-900 flex flex-col selection:bg-neutral-800 selection:text-white">
      ...
      {isModalOpen && (
        <ImageHistoryModal
          isOpen={isModalOpen}
          onClose={closeSubmittedHistoryModal}
      ...
      {isGeneratedHistoryModalOpen && (
         <GeneratedImageHistoryModal
          ...
          activeGeneratingReportId={activeImageForReport?.id || null}
        />
      )}
      {isLoginModalOpen && (
